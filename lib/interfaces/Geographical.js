...
class Geographical extends MultidimensionalInterface {
    ...
    async selectTile(latitude, longitude, triples) {
        ...
        if (path) {
            ...
            // We extract the values and its sensor to update the average (in the buffer)
    ...
    // Function to update the average in a specific tile (in buffer)
    ...
    // Updating average in the file
    // Herefore we iterate through our buffer and if there's a match in the file we update the average and its sample size
    async updateAverageFile() {
        if (!Utils.exists(this.fragmentsPath + '/tileAverage.trig')) {
            ...
            let tempBuffer = this.averageBuffer;
            this.averageBuffer.clear();

            for (let tile of tempBuffer.keys()) {
