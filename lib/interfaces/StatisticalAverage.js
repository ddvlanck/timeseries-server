...
class StatisticalAverage extends MultidimensionalInterface {
    ...
    async handleRequest(ctx, filePath, metadata) {
        if (!Utils.exists(filePath)) {
            ...
            //let staticTriples = await Utils.formatTriples('application/trig', st, {});
        ...
        for (let source of newValues.keys()) {
            ...
            for (let i in oldTriples) {
                ...
                if (source === oldTriples[i].subject) {
                    ...
                    if (oldTriples[i].predicate === 'ts:mean') {
                        oldTriples[i].object = '"' + this.calculateMean(parseFloat((newValues.get(source))),
                            parseFloat(oldTriples[i].object), parseInt(Utils.getLiteralValue(sampleValues.get(source)))) + '"';
