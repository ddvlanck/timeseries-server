...
class RawData extends MultidimensionalInterface {
    ...
    async onData(data) {
        ...
        this.lastGat = await Utils.getResultTimeValue(this.latestData);
    ...
    setupPollInterfaces() {
        ...
        super.commMan.router.get('/' + this.name + '/fragments', async (ctx, next) => {
            ...
            let staticTriples = await Utils.formatTriples('application/trig', st, pr[0]);

            let fragmentTriples = await Utils.formatTriples('application/trig', ft[1], pr[0]);
