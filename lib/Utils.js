...
module.exports = new class Utils {
    ...
    getFragmentsForTimeRange(path, fragmentNumber) {
        ...
        for (let i = 0; i < files.length; i++) {
            if (files[i].indexOf(timeRange + '_fragment') >= 0) {
                ...
                if(fragmentNumber){
                    let number = parseInt(files[i].substring(files[i].indexOf('_fragment') + 10, files[i].indexOf('.trig')));

                    if(fragmentNumber === number){
                        console.log("YEEEEEEEEEES");
                        index = fragmentNumber - 1;
                    }
                }
            }
        }

        if(!fragmentNumber && result.length > 1){
    ...
    getFragmentsForTile(filePath, fragmentNumber) {
        ...
        let tile = filePath.substring(filePath.lastIndexOf('/') + 1, filePath.indexOf('.trig'));
        let index = -1;
        let result = [];
        let tileCounter = 0;
        ...
        for (let i = 0; i < files.length; i++) {
            if (files[i].indexOf(tile) >= 0) {
                ...
                if (fragmentNumber) {
                    let number = files[i].substring(files[i].indexOf('_fragment') + 10, files[i].indexOf('.trig'));
                    index = number == fragmentNumber ? tileCounter : -1;
                }
            }
        }
    ...
    getTileAverage(coordinates) {
        coordinates = coordinates.replace(/_/g, '/');
        return new Promise(async resolve => {
            const triples = (await this.getTriplesFromFile('./example/data/Geographical/tileAverage.trig'))[1];
            for(let i = 0 ; i < triples.length ; i++){
                if(triples[i].predicate === 'ts:mean' && triples[i].subject === 'Tile/' + coordinates){
                    resolve(triples[i].object);
                }
            }
