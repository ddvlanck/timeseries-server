...
module.exports = new /** @class */ (function () {
    ...
    Utils.prototype.overwriteFile = function (path, data) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, fs.writeFile(path, data, (function (err) {
                            if (err)
                                throw err;
                        }))];
                    case 1: return [2 /*return*/, _a.sent()];
    ...
    * */
    Utils.prototype.getFilesFromInterfaceByParams = function (path) {
        var files = [];
    ...
    Utils.prototype.getAllDataToGenerateFile = function (path, files) {
        var level = null;
        var date = path.substring(path.lastIndexOf('/') + 1, path.length - 5);
        var timePieces = date.split('-');
        ...
        // f = f.replace(/_/g, ':');
        var monthRegex = new RegExp('^' + date + '-[0-1][0-9].trig$');
        var dayRegex = new RegExp('^' + date + '-[0-3][0-9].trig$');
        var hourRegex = new RegExp('^' + date + 'T[0-2][0-9].trig');
        var counter = 0;
        var dataFiles = [];
    ...
    Utils.prototype.getFilesForDay = function (regex) {
    };
    return Utils;
}());
